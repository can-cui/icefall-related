...
def remove_space(text_list):
    ...
    for text in text_list:
        ...
        clean_list.append(cleaned_text)
    return clean_list
...
def compute_loss(
    ...
    texts_rich=remove_space(texts_rich)
    # print('texts_rich')
    # print(texts_rich)
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if not torch.all(is_finite):
            ...
            #     ~pruned_loss_is_finite) or torch.all(~ctc_loss_is_finite):
...
def run(rank, world_size, args):
    ...
    ami_train_cuts = filter_utterances_on_max_chars_per_sec(ami_train_cuts, 18.0) # 1.5*12 chars per sec
