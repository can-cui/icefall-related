...
export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/miniconda3/envs/dict/lib"
...
CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=$id python3 -u lstm_transducer_stateless3_ctc/train_noCtc.py --world-size 4 --num-epochs 40 --start-epoch 1 --exp-dir lstm_transducer_stateless3_ctc/exp-ami-scratch-noCtc/ --max-duration 750 --num-encoder-layers 6 --bpe-model data/lang_bpe_500/bpe.model --use-fp16 1 --ctc-loss-scale 0 2>&1 | tee lstm_transducer_stateless3_ctc/exp-ami-scratch-noCtc/train.log
