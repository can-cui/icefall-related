...
def filter_commented_utterances(cuts: CutSet) -> CutSet:
    def remove_commented_utt(c: Cut):
        return '[' not in c.supervisions[0].text

    cuts = cuts.filter(remove_commented_utt)
...
def run(rank, world_size, args):
    ...
    train_cuts = filter_short_and_long_utterances(train_cuts, 1.0, 20.0)
    if params.full_libri:
        train_cuts += filter_short_and_long_utterances(librispeech.train_clean_360_cuts(), 1.0, 20.0)
        train_cuts += filter_short_and_long_utterances(librispeech.train_other_500_cuts(), 1.0, 20.0)

    cv = CV(manifest_dir=args.manifest_dir)
    cv_train_cuts = cv.train_cuts()
    cv_train_cuts = filter_short_and_long_utterances(cv_train_cuts, 1.0, 17.0)
    cv_train_cuts = filter_utterances_on_max_chars_per_sec(cv_train_cuts, 18.0)
    ...
    ami_len=len(ami_train_cuts.to_eager())
    logging.info(f"libri_len, cv_len, ami_len: {libri_len} , {cv_len} , {ami_len}")

    cuts_train = CutSet.mux(
        train_cuts,
        cv_train_cuts,
        ami_train_cuts,
        weights=[libri_len, cv_len,ami_len]
    )
