...
import torch
from model_e2e import Transducer
...
def greedy_search(
    ...
    hyp=get_hyp(model.decoder_common)
...
def greedy_search_batch(
    ...
    def get_ans(decoder,encoder_out,joiner):
      blank_id = decoder.blank_id
      unk_id = getattr(model, "unk_id", blank_id)
      context_size = decoder.context_size
      ...
      # print(decoder_input.shape) # torch.Size([14, 2])
      id_list=[[mode_id] * context_size for _ in range(N)]
      mode_input = torch.tensor(
          id_list,
          device=device,
          dtype=torch.int64,
      ) 
      # print("mode_input.shape")
      # print(mode_input.shape)
      globle_input=torch.stack((decoder_input, mode_input), dim=2)

      decoder_out = decoder(globle_input, need_pad=False)
      ...
      for batch_size in batch_size_list:
          ...
          if emitted:
              ...
              id_list=[[mode_id for _ in row] for row in decoder_input]
              mode_input = torch.tensor(
                  id_list,
                  device=device,
                  dtype=torch.int64,
      ...
      return ans
    # decoder_type="uncase"
    # if decoder_type=="uncase":
    #   ans=get_ans(model.decoder_rich,encoder_out,model.joiner_rich)
    # elif decoder_type=="common":
    ans=get_ans(model.decoder,encoder_out,model.joiner)
    # elif decoder_type=="uncase":
    #   ans=get_ans(model.decoder_uncase,encoder_out,model.joiner_uncase)
    return ans
