...
from mls import LibriSpeech
from cv import CV
#from mtedx import Mtedx
...
def display_and_save_batch_nonsp(
    batch: dict,
    ...
    with torch.set_grad_enabled(is_training):
        ...
        if not torch.all(is_finite):
            ...
            if sp is not None:
            	...
            	display_and_save_batch_nonsp(batch, params=params)
...
def run(rank, world_size, args):
    ...
    train_cuts = filter_short_and_long_utterances(train_cuts, 1.0, 23.0)
    train_cuts = filter_utterances_on_max_chars_per_sec(train_cuts, 24.0) # 2*12 chars per sec

    cv = CV(manifest_dir=args.manifest_dir)
    cv_train_cuts = cv.train_cuts()
    cv_train_cuts = filter_short_and_long_utterances(cv_train_cuts, 1.0, 10.0)
    cv_train_cuts = filter_utterances_on_max_chars_per_sec(cv_train_cuts, 24.0) # 2*12 chars per sec
    
    #ted = Mtedx(manifest_dir=args.manifest_dir)
    #ted_train_cuts = ted.train_cuts()
    #ted_train_cuts = filter_short_and_long_utterances(ted_train_cuts, 1.0, 8.0)
    #ted_train_cuts = filter_utterances_on_max_chars_per_sec(ted_train_cuts, 24.0) # 2*12 chars per sec
        
    ### use the following or better call local/display_manifest_statistics.py
    #libri_len = len(train_cuts.to_eager())
    #cv_len = len(cv_train_cuts.to_eager())
    #ted_len = len(ted_train_cuts.to_eager())
    #logging.info(f"libri_len, cv_len: {libri_len} , {cv_len}")
    
    cuts_train = CutSet.mux(
        train_cuts,
        cv_train_cuts,
        #ted_train_cuts,
        weights=[1122540 , 92464]# [libri_len, cv_len] 
    )
    ...
    train_dl = asr_datamodule.train_dataloaders(cuts_train, sampler_state_dict=sampler_state_dict)

    valid_cuts = librispeech.dev_cuts()
    valid_cuts += cv.dev_cuts()
    #valid_cuts += ted.dev_cuts()
