...
from asr_datamodule_ami import AsrDataModule
import re
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="""Type of mode: 0 stands for common transcription,
        1 stands for rich transcription""",
    )
...
def filter_short_and_long_utterances(cuts: CutSet, low: float = 1.0) -> CutSet:
    def remove_short_and_long_utt(c: Cut):
        # Keep only utterances with duration between 1 second and 17 seconds
        return low < c.duration
...
def main():
    ...
    ami = AMI()
    ami_test = ami.test_cuts()

    ami_test = filter_short_and_long_utterances(ami_test, 0.1)
    asr_datamodule = AsrDataModule(args)
    test_dl = asr_datamodule.test_dataloaders(ami_test)
