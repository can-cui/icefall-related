...
from train_uncase_case import add_model_arguments, get_params, get_transducer_model
...
class OnnxEncoder(nn.Module):
    ...
    def forward(
        ...
        return encoder_proj_out, encoder_out_lens, new_h, new_c
...
def export_encoder_model_onnx(
    ...
    h = torch.rand(
        encoder_model.encoder.num_encoder_layers, N, encoder_model.encoder.d_model
    )
    c = torch.rand(
        encoder_model.encoder.num_encoder_layers,
        N,
        encoder_model.encoder.rnn_hidden_size,
    )
...
def main():
    ...
    print(
        "scaled model.encoder.parameters()",
        sum([p.numel() for p in model.encoder.parameters()]),
    )

    encoder = OnnxEncoder(
        encoder=model.encoder,
        encoder_proj=model.joiner_rich.encoder_proj,
        # ctc_model=model.ctc_model
    )

    decoder = OnnxDecoder(
        decoder=model.decoder_rich,
        decoder_proj=model.joiner_rich.decoder_proj,
    )

    joiner = OnnxJoiner(output_linear=model.joiner_rich.output_linear)
    ...
    decoder_filename = params.exp_dir / f"decoder-rich-{suffix}.onnx"
    ...
    joiner_filename = params.exp_dir / f"joiner-rich-{suffix}.onnx"
